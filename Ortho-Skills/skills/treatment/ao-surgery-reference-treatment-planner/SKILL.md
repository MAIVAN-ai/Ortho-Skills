# Skill: AO Surgery Reference Treatment Planner

## Skill ID

`ao-surgery-reference-treatment-planner`
...
Use the confirmed or draft diagnosis and classification to structure an AO Surgery Reference-based treatment-options summary for clinician review.
...
1. Receive confirmed/draft classification and region.
2. Call AO Surgery Reference query skill.
3. Separate nonoperative and operative categories.
4. Summarise indications and contraindications at a high level.
5. Extract aftercare considerations as links and summaries.
6. State that surgeon judgement and local protocols govern final decisions.
...
```yaml
skill_output:
  skill_id: "ao-surgery-reference-treatment-planner"
...
Use AO Surgery Reference as a web-source lookup target: <https://surgeryreference.aofoundation.org/>. Do not copy AO content. Summarise and link.

## Example Prompt

```text
Use the AO Surgery Reference Treatment Planner skill for this orthopaedic case. Provide a structured draft output and state uncertainty.
```
